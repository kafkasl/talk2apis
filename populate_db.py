...
from database.services import Services, ServiceCategories, ServiceAPIs, APIEndpoints,  APIParameters, DeleteService
...
def main(files, recreate=False):
    ...
    for file in files:
        ...
        try:
            service = session.query(Services).filter(Services.name == service_name).one()
            if not recreate:
                continue

            print(f"Service {service_name} already exists in the database. Recreating...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('definition_files', nargs='*', help='A list of definition files')
    parser.add_argument('--recreate', action='store_true', help='Overwrite db entries for existing apis')
