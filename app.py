...
def llm_get_prompt_tags(service, user_prompt):
    prompt = f"""
    Given the API for a service and a user prompot reply with the list of tags should be 
...
def chat():
    ...
    if 'service' not in data or data['service'] != 'github':
        response = {
            'error': 'Bad parameters: missing or invalid service'
        }
        return jsonify(response), 400

    if 'token' not in data:
        response = {
            'error': 'Bad parameters: missing token'
        }
        return jsonify(response), 400
    
    if 'prompt' not in data or len(data['prompt']) == 0:
        response = {
            'error': 'Bad parameters: missing or invalid prompt'
        }
        return jsonify(response), 400
    
    code, error = process_user_prompt()
